...
# Generate a series of blue colors from light to dark
num_colors = len(all_approaches)  # Adjust for more or fewer shades
cmap = plt.get_cmap('Blues')  # Use the 'Blues' colormap
colors = [cmap(i / (num_colors - 1)) for i in range(num_colors)][::-1] # Make sure the lightest color is the kvlink-31-1 one
markers = ['*'] * len(all_approaches)
...
if __name__ == "__main__":
    ...
    axs[0][0].legend(loc="upper center", bbox_to_anchor=(1.5, 1.6), ncol=4, fontsize=22)
    ...
    # Save
    plt.savefig("results/fig-eval-init-final-ratio.pdf", format="pdf", bbox_inches="tight", dpi=300)