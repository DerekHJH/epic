...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if cache_fuse_metadata["breakdown"] and attn_metadata.num_prefill_tokens > 0:
        ...
        attn_output = self.attn(q, k, v, kv_cache, attn_metadata,
                                ...
                                self.kv_scale)
        if cache_fuse_metadata["breakdown"] and attn_metadata.num_prefill_tokens > 0:
            cache_fuse_metadata["attntime"].append(time.perf_counter()-start)
...
class LlamaModel(nn.Module):
    ...
    def __init__(
        ...
        self.cache_fuse_metadata = {"check_layers":[1],
                                    ...
                                    "org_seq_len": None,
                                    "collect": False,
                                    "kvlink": None, # None: turned off; !=None: turned on ([...]: position that needs recompute; []: second stage)
                                    "prob_attn": False,
                                    "attn_matrix": [],
                                    "layer_idx": 0,
                                    "attntime": [],
    ...
    def forward(
        ...
        if attn_metadata.prefill_metadata:
            ...
            if self.cache_fuse_metadata["kvlink"] is not None:
                self.cache_fuse_metadata["org_seq_len"] = input_ids.shape[0] 
                self.cache_fuse_metadata["fake_q"] = None  
                self.cache_fuse_metadata["attn_bias"] = None
                self.cache_fuse_metadata["original_slot_mapping"] = None
                ...
                self.cache_fuse_metadata["imp_indices"] = self.cache_fuse_metadata["kvlink"]
                positions = positions[self.cache_fuse_metadata["imp_indices"]]
                hidden_states = hidden_states[self.cache_fuse_metadata["imp_indices"]]
