...
class XFormersImpl(AttentionImpl):
    ...
    def forward(
        ...
        if cache_fuse_metadata["kvlink"] is not None and len(cache_fuse_metadata["kvlink"]) > 0:
        ...
        if cache_fuse_metadata["kvlink"] is not None:
            key_old[cache_fuse_metadata["imp_indices"]] = key 
            value_old[cache_fuse_metadata["imp_indices"]] = value
            key = key_old
            value = value_old
        ...
        if kv_cache is not None:
            ...
            if cache_fuse_metadata["prob_attn"]:
                if attn_metadata.num_prefill_tokens > 0: # Prefill stage
                    cache_fuse_metadata['attn_matrix'].append([])

                elif attn_metadata.num_prefill_tokens == 0: # decode stage
