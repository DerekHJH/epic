...
class XFormersImpl(AttentionImpl[XFormersMetadata]):
    ...
    def forward(
        ...
        * For cross-attention:
        ...
        if cache_fuse_metadata["kvlink"] is not None:
            key_old = old_kv[0].view(-1, self.num_kv_heads, self.head_size)
            value_old = old_kv[1].view(-1, self.num_kv_heads, self.head_size)
            if len(cache_fuse_metadata["kvlink"]) > 0:
                if self.num_queries_per_kv == 1:
                    # multi-head attention
                    attn_bias = _make_partial_bias_mha(cache_fuse_metadata,
                        query.device, self.num_kv_heads)
                else:
                    # group query attention
                    attn_bias = _make_partial_bias_gqa(cache_fuse_metadata,
                        query.device, self.num_kv_heads, self.num_queries_per_kv)
                cache_fuse_metadata["attn_bias"] = attn_bias
                attn_metadata.prefill_metadata.attn_bias = None
                cache_fuse_metadata["kvlink"] = [] # only compute once
            # update KV Cache
        ...
        if status in [1, 2]:
            ...
            value_old = old_kv[1].view(-1, self.num_kv_heads, self.head_size)
            if status in [1]:
                topk_num = int((value.shape[0]-1)*cache_fuse_metadata["recomp_ratio"])
                temp_diff = torch.sum((value[:-1,:,:]-value_old[:-1,:,:])**2, dim=[1, 2])
                top_indices = torch.topk(temp_diff, k=topk_num).indices
                top_indices = torch.cat([top_indices, torch.tensor([query.shape[0]-1], device=top_indices.device)])
        ...
        if (attn_type != AttentionType.ENCODER and kv_cache.numel() > 0):
            ...
            if (key is not None) and (value is not None):
                ...
                if cache_fuse_metadata["prob_attn"]:
                    if attn_metadata.num_prefill_tokens == 0: # decode stage
                        old_kv[0] = torch.concat((old_kv[0], key.view(1, -1)), dim=0)
                        old_kv[1] = torch.concat((old_kv[1], value.view(1, -1)), dim=0)
                    else:
                        cache_fuse_metadata['attn_matrix'].append([])
                    Q = query.view(query.shape[0], self.num_kv_heads,
                            self.num_queries_per_kv, query.shape[-1]).permute(1, 2, 0, 3)
                    K = old_kv[0].view(old_kv[0].shape[0], self.num_kv_heads, -1)
                    K = K[:, :, None, :].expand(K.shape[0], self.num_kv_heads,
                            self.num_queries_per_kv, K.shape[-1]).permute(1, 2, 3, 0)
                    A = torch.matmul(Q, K)
                    cache_fuse_metadata['attn_matrix'][cache_fuse_metadata['layer_idx']].append(A)
                    cache_fuse_metadata['layer_idx'] += 1
                    # NOTE(Shiju): beware of the number 32
                    cache_fuse_metadata['layer_idx'] %= 32
        (num_prefill_query_tokens, num_prefill_kv_tokens,
        num_decode_query_tokens) = \
            get_num_prefill_decode_query_kv_tokens(attn_metadata, attn_type)

        output = torch.empty_like(query)
        if cache_fuse_metadata["kvlink"] is not None or status in [1, 2]:
            num_prefill_query_tokens, num_decode_query_tokens = len(query), 0
        ...
        if prefill_meta := attn_metadata.prefill_metadata:
            ...
            if kv_cache.numel() == 0 or prefill_meta.block_tables.numel() == 0:
                ...
                out = self._run_memory_efficient_xformers_forward(
                    query, key, value, prefill_meta, status, cache_fuse_metadata, attn_type=attn_type)
    ...
    def _run_memory_efficient_xformers_forward(
        ...
        if self.alibi_slopes is None:
            ...
            if status in [1, 2] or cache_fuse_metadata["kvlink"] is not None:
                attn_bias[0] = cache_fuse_metadata["attn_bias"]
...
def _make_partial_bias_gqa(cache_fuse_metadata, 
    ...
    """ attention mask for tokens. 
        FIXME(Shiju): Now only focus on batch size = 1.
    """
...
def _make_partial_bias_mha(cache_fuse_metadata, 
                       device,
                       num_kv_heads):
    """ attention mask for tokens. 
        FIXME(Shiju): Now only focus on batch size = 1.
    ...
    attn_mask = torch.triu(torch.ones(padded_len,
                                      ...
                                      device=device),
                           diagonal=1)
    attn_mask = (attn_mask * torch.finfo(dtype).min).view(1, 1, padded_len, padded_len)
    attn_mask = attn_mask[:, :, imp_indices]
