...
class EvalConfigs:
    ...
    all_approaches: List[str] = field(
        default_factory = lambda: ['fr', 'naive', 'cacheblend-15', 'kvlink-1']
...
class EvalEngine:
    ...
    def run(self):
        ...
        self.model: AutoModelForCausalLM = LLM(
            ...
            enforce_eager=True, # Needed to prob attention for better hacking
        )
    ...
    def run_inference(self, configs: EvalConfigs, dataset: Data_set, tokenizer: AutoTokenizer, llm: LLM) -> Data_set:
        ...
        for _, (system_prompts, mod_prompts, free_form_prompt) in tenumerate(dataset, desc="dataset", leave=True):
            ...
            cache_fuse_metadata["prob_attn"] = True
            ...
            torch.save(cache_fuse_metadata["attn_matrix"], os.path.join(configs.result_path, f"attn_matrix_{configs.approach}.pt"))
            torch.save(start_offset, os.path.join(configs.result_path, f"start_offset_{configs.approach}.pt"))
