...
if __name__ == "__main__":
    ...
    for i, dataset_name in enumerate(all_datasets):
        ...
        for j, model_name in enumerate(all_models):
            ...
            # plot algo-0
            path0 = f"../zero-linking-overhead/results/{dataset_name}/{last_model_name}/data.json"
            dataset0 = pd.read_json(path0)
            ...
            # plot algo-0
            x_metric = np.mean(dataset0[f"TTFT_kvlink-0"])
            y_metric = np.mean(dataset0[f"score_kvlink-0"])
            max_y_metric = max(max_y_metric, y_metric)
            max_x_metric = max(max_x_metric, x_metric)
            axs[i][j].scatter(
                x_metric,
                y_metric,
                # label=approach_names_map["kvlink-0"],
                s=200,
                c="black",
                marker="*",
            )
    ...
    # Save
    plt.savefig("results/fig-eval-e2e.pdf", format="pdf", bbox_inches="tight", dpi=300)