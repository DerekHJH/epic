...
class EvalConfigs:
    ...
    all_approaches: List[str] = field(
        default_factory = lambda: ["kvlink-1-31", "kvlink-2-30", "kvlink-4-28", "kvlink-8-24", "kvlink-16-16", "kvlink-24-8", "kvlink-28-4", "kvlink-30-2", "kvlink-31-1"]
...
class EvalEngine:
    ...
    def run_inference(self, configs: EvalConfigs, dataset: Data_set, tokenizer: AutoTokenizer, llm: LLM) -> Data_set:
        ...
        for _, (system_prompts, mod_prompts, free_form_prompt) in tenumerate(dataset, desc="dataset", leave=True):
            ...
            # We only leave the kvlink approach here
            if 'kvlink' in configs.approach:
                ...
                and the last Q
                """
                num_init = int(configs.approach.split('-')[1])
                num_final = int(configs.approach.split('-')[2])
                temp = []
                # Compute boundary
                for i in range(1, len(start_offset)-1):
                    if i == 1:
                        temp += list(range(start_offset[i], start_offset[i]+num_init))
                    # We compute all query tokens
                    elif i == len(start_offset)-2:
                        temp += list(range(start_offset[i]-num_final, start_offset[i+1]))
                    else:
                        temp += list(range(start_offset[i]-num_final, start_offset[i]+num_init))
