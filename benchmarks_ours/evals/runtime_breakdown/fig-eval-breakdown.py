import matplotlib.pyplot as plt
import pandas as pd
import numpy as np
import os
from typing import List, Union
import string
import json
from benchmarks_ours.evals.utils import model_names_map, dataset_names_map, dataset_metrics_map, approach_names_map


all_datasets = ['2wikimqa', 'musique', 'samsum', 'multi_news', 'hotpotqa', 'needle'][::-1]
all_models = ["mistralai/Mistral-7B-Instruct-v0.2", "meta-llama/Meta-Llama-3.1-8B-Instruct", "01-ai/Yi-Coder-9B-Chat"] 


if __name__ == "__main__":

    fig, axs = plt.subplots(
        len(all_datasets), len(all_models), figsize=(6 * len(all_models), 5 * len(all_datasets))
    )

    for i, dataset_name in enumerate(all_datasets):
        max_y_metric = 0
        for j, model_name in enumerate(all_models):
            last_model_name = model_name.split("/")[-1]
            path = f"results/{dataset_name}/{last_model_name}/data.json"
            dataset = pd.read_json(path)

            num_bars = len(dataset["attntime_cacheblend-15"][0])

            data = np.zeros(num_bars)
            for row in range(len(dataset["attntime_cacheblend-15"])):
                data = data + np.array(dataset[f"attntime_cacheblend-15"][row])
            
            data = data / len(dataset["attntime_cacheblend-15"])
            data = data * 1000  # ms

            runtime_overhead = (data[1]-data[0]) / data.sum() * 100
        
            bars = axs[i][j].bar(np.arange(0, num_bars, 1), data, color='skyblue', edgecolor='black')

            height = bars[1].get_height()
            axs[i][j].text(
                bars[len(bars) // 2].get_x(),
                bars[1].get_height() / 2,
                f"Runtime overhead\n{runtime_overhead:.2f}%",
                ha="center",
                va="bottom",
                fontsize=24,
            )
            # for bar in bars:
            #     height = bar.get_height()
            #     axs[i][j].text(
            #         bar.get_x() + bar.get_width() / 2,
            #         height,
            #         f"{height:.2f}",
            #         ha="center",
            #         va="bottom",
            #         # fontsize=20,
            #     )

            max_y_metric = max(max_y_metric, axs[i][j].get_ylim()[1])
        
        # After max_x_metric and max_y_metric are determined
        max_y_metric *= 1.05
        for j, model_name in enumerate(all_models):
            axs[i][j].set_ylim(bottom=0, top=max_y_metric)
            axs[i][j].tick_params(axis="x", labelsize=20)
            axs[i][j].tick_params(axis="y", labelsize=20)


    # Draw model names
    for j, model_name in enumerate(all_models):
        axs[0][j].set_title(model_names_map[model_name], fontsize=24)

    # Draw x metric name
    for j in range(1, len(all_models) + 1):
        axs[-1][-j].set_xlabel("Layer ID", fontsize=24)

    for i, dataset_name in enumerate(all_datasets):
        axs[i][0].set_ylabel("time / ms", fontsize=24)
        axs[i][0].text(
            -0.3,
            0.5,
            dataset_names_map[dataset_name],
            fontsize=24,
            rotation="vertical",
            ha="left",
            va="center",
            transform=axs[i][0].transAxes,
        )

    # Save
    plt.savefig("results/fig-eval-breakdown.pdf", format="pdf", bbox_inches="tight", dpi=300)