...
class EvalConfigs:
    ...
    all_approaches: List[str] = field(
        default_factory = lambda: ['cacheblend-15']
...
class EvalEngine:
    ...
    def run_inference(self, configs: EvalConfigs, dataset: Data_set, tokenizer: AutoTokenizer, llm: LLM) -> Data_set:
        ...
        for _, (system_prompts, mod_prompts, free_form_prompt) in tenumerate(dataset, desc="dataset", leave=True):
            ...
            if 'cacheblend' in configs.approach:
            ...
            cache_fuse_metadata["breakdown"] = True
            cache_fuse_metadata["attntime"] = []
            ...
            cache_fuse_metadata["breakdown"] = False
            results[f'attntime_{configs.approach}'].append(cache_fuse_metadata['attntime'])
