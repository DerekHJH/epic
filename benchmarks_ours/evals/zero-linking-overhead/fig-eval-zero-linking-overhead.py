...
colors = ['#0076A8', '#2A9BD5', '#4DA6D6', '#7FB3D5', '#A3C1E0']
markers = ['*'] * 5
...
if __name__ == "__main__":
    ...
    for i, dataset_name in enumerate(all_datasets):
        ...
        for j, model_name in enumerate(all_models):
            ...
            path = f"../e2e/results_e2e/{dataset_name}/{last_model_name}/data.json"
            ...
            x_metric = np.mean(dataset[f"TTFT_kvlink-0"])
            y_metric = np.mean(dataset[f"score_kvlink-0"])
            max_y_metric = max(max_y_metric, y_metric)
            max_x_metric = max(max_x_metric, x_metric)
            axs[i][j].scatter(
                x_metric,
    ...
    # Save
    plt.savefig("results/fig-eval-zero-linking-overhead.pdf", format="pdf", bbox_inches="tight", dpi=300)